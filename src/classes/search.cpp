...
void Search::do_constrained_search(Protein* protein, Molecule* ligand)
{
    ...
    cs_res[cs_idx] = protein->get_residue(cs_res[cs_idx]->get_residue_no());
    cs_lag[cs_idx]->update_atom_pointers(ligand);
    ...
    if (cs_bt[cs_idx] != pi && cs_bt[cs_idx] != mcoord)
    {
        ...
        cs_res[cs_idx]->conform_atom_to_location(cs_res[cs_idx]->get_one_most_bindable(cs_bt[cs_idx])->name, loneliest);
    ...
    Point resna;
    Atom* resbba = mtl ? mtl : cs_res[cs_idx]->get_one_most_bindable(cs_bt[cs_idx]);
    ...
    // cout << *cs_lag[cs_idx] << " to " << cs_res[cs_idx]->get_name() << ":" << resbba->name << " @ " << resbba->get_location() << endl << endl;
    ...
    // Rotate the ligand so that its barycenter aligns with the "loneliest" point.
    ligand->movability = MOV_ALL;
    agcen = ligbba->get_location();
    ...
    for (j=0; j<10; j++)
    {
        // Point the residue's "best" atom at the ligand group.
        agcen = ligbba->get_location();
        cs_res[cs_idx]->conform_atom_to_location(resbba->name, agcen);
    ...
    if (cs_bt[cs_idx] != pi)
    {
        ...
        cs_res[cs_idx]->maintain_contact(resbba, ligbba);
    }
        ...
        for (j=0; j<3; j++)
        {
            for (; theta < M_PI*2; theta += cs_360_step)
            {
                ...
                float f = ligand->get_intermol_clashes(reinterpret_cast<Molecule**>(cc))
                    - 100.0*ligand->get_intermol_binding(reinterpret_cast<Molecule**>(cc), false)
                    ;
