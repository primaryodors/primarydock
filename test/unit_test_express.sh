...
test/group_test_mol "c1ccccc1CCO" | sed '/^#/d' | sort -d > test/received/groups.$MOLECULE.received.txt
...
MOLECULE="indole"
test/group_test_mol "C12=C(C=CN2)C=CC=C1" | sed '/^#/d' | sort -d > test/received/groups.$MOLECULE.received.txt
RESULT=$(diff --unified test/groups.$MOLECULE.approved.txt test/received/groups.$MOLECULE.received.txt)
if [ -z "$RESULT" ]; then
    printf "${GRN}Molecule group test succeeded for $MOLECULE.${NC}\n"
else
...
MOLECULE="pyrazine"
test/group_test_mol "n1ccncc1" | sed '/^#/d' | sort -d > test/received/groups.$MOLECULE.received.txt
RESULT=$(diff --unified test/groups.$MOLECULE.approved.txt test/received/groups.$MOLECULE.received.txt)
if [ -z "$RESULT" ]; then
    printf "${GRN}Molecule group test succeeded for $MOLECULE.${NC}\n"
else
...
MOLECULE="cinnamaldehyde"
test/group_test_mol "c1ccccc1C=CC=O" | sed '/^#/d' | sort -d > test/received/groups.$MOLECULE.received.txt
RESULT=$(diff --unified test/groups.$MOLECULE.approved.txt test/received/groups.$MOLECULE.received.txt)
if [ -z "$RESULT" ]; then
    printf "${GRN}Molecule group test succeeded for $MOLECULE.${NC}\n"
else
...
MOLECULE="arabinose"
test/group_test_mol "C1[C@@H]([C@@H]([C@H](C(O1)O)O)O)O" | sed '/^#/d' | sort -d > test/received/groups.$MOLECULE.received.txt
RESULT=$(diff --unified test/groups.$MOLECULE.approved.txt test/received/groups.$MOLECULE.received.txt)
if [ -z "$RESULT" ]; then
    printf "${GRN}Molecule group test succeeded for $MOLECULE.${NC}\n"
else
...
MOLECULE="adenosine"
test/group_test_mol "n2c1c(ncnc1n(c2)[C@@H]3O[C@@H]([C@@H](O)[C@H]3O)CO)N" | sed '/^#/d' | sort -d > test/received/groups.$MOLECULE.received.txt
RESULT=$(diff --unified test/groups.$MOLECULE.approved.txt test/received/groups.$MOLECULE.received.txt)
if [ -z "$RESULT" ]; then
    printf "${GRN}Molecule group test succeeded for $MOLECULE.${NC}\n"
else
