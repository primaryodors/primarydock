...
$version = filemtime("method_acv1st.php");

$json_file = "dock_results_acv1st.json";
$dock_data = [];
...
foreach ($dock_data as $rcp => $ligs)
{
    foreach ($ligs as $ligand => $pair)
    {
        ...
        $kJmola = @floatval($pair['Node 0']) ?: 0;
        $kJmoli = @floatval($pair['Node 1']) ?: 0;

        $yvals[$rcp]["acv.e"][$idx] = $kJmola;
        $yvals[$rcp]["acv.d"][$idx] = $kJmola - $kJmoli;
