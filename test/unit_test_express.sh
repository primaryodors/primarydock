...
RESULT=$(diff --unified test/brot.$MOLECULE.approved.txt testdata/received/brot.$MOLECULE.received.txt)
if [ -z "$RESULT" ]; then
    printf "${GRN}Rotatable bond test succeeded for $MOLECULE.${NC}\n"
else
    printf "${RED}Rotatable bond test FAILED for $MOLECULE.${NC}\n"
    diff --color --unified test/brot.$MOLECULE.approved.txt testdata/received/brot.$MOLECULE.received.txt
fi
...
test/group_test_mol "c1ccccc1CCO" | sed '/^#/d' | sort > testdata/received/groups.$MOLECULE.received.txt
RESULT=$(diff --unified test/groups.$MOLECULE.approved.txt testdata/received/groups.$MOLECULE.received.txt)
if [ -z "$RESULT" ]; then
    printf "${GRN}Molecule group test succeeded for $MOLECULE.${NC}\n"
else
    printf "${RED}Molecule group test FAILED for $MOLECULE.${NC}\n"
    diff --color --unified test/groups.$MOLECULE.approved.txt testdata/received/groups.$MOLECULE.received.txt
fi

MOLECULE="indole"
test/group_test_mol "C12=C(C=CN2)C=CC=C1" | sed '/^#/d' | sort > testdata/received/groups.$MOLECULE.received.txt
RESULT=$(diff --unified test/groups.$MOLECULE.approved.txt testdata/received/groups.$MOLECULE.received.txt)
if [ -z "$RESULT" ]; then
    printf "${GRN}Molecule group test succeeded for $MOLECULE.${NC}\n"
else
...
MOLECULE="pyrazine"
test/group_test_mol "n1ccncc1" | sed '/^#/d' | sort > testdata/received/groups.$MOLECULE.received.txt
RESULT=$(diff --unified test/groups.$MOLECULE.approved.txt testdata/received/groups.$MOLECULE.received.txt)
if [ -z "$RESULT" ]; then
    printf "${GRN}Molecule group test succeeded for $MOLECULE.${NC}\n"
else
...
MOLECULE="cinnamaldehyde"
test/group_test_mol "c1ccccc1C=CC=O" | sed '/^#/d' | sort > testdata/received/groups.$MOLECULE.received.txt
RESULT=$(diff --unified test/groups.$MOLECULE.approved.txt testdata/received/groups.$MOLECULE.received.txt)
if [ -z "$RESULT" ]; then
    printf "${GRN}Molecule group test succeeded for $MOLECULE.${NC}\n"
else
...
MOLECULE="arabinose"
test/group_test_mol "C1[C@@H]([C@@H]([C@H](C(O1)O)O)O)O" | sed '/^#/d' | sort > testdata/received/groups.$MOLECULE.received.txt
RESULT=$(diff --unified test/groups.$MOLECULE.approved.txt testdata/received/groups.$MOLECULE.received.txt)
if [ -z "$RESULT" ]; then
    printf "${GRN}Molecule group test succeeded for $MOLECULE.${NC}\n"
else
...
MOLECULE="adenosine"
test/group_test_mol "n2c1c(ncnc1n(c2)[C@@H]3O[C@@H]([C@@H](O)[C@H]3O)CO)N" | sed '/^#/d' | sort > testdata/received/groups.$MOLECULE.received.txt
RESULT=$(diff --unified test/groups.$MOLECULE.approved.txt testdata/received/groups.$MOLECULE.received.txt)
if [ -z "$RESULT" ]; then
    printf "${GRN}Molecule group test succeeded for $MOLECULE.${NC}\n"
else
...
RESULT=$(diff --unified test/group_test_res.approved.txt testdata/received/group_test_res.received.txt)
if [ -z "$RESULT" ]; then
    printf "${GRN}Residue group test succeeded.${NC}\n"
else
    printf "${RED}Residue group test FAILED.${NC}\n"
    diff --color --unified test/group_test_res.approved.txt testdata/received/group_test_res.received.txt
fi
