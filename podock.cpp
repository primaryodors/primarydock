...
void iteration_callback(int iter)
{
	...
	if (gcfmols && seql)
	{
		...
		for (i=0; i<sphres; i++)
		{
			discrete[i+j].paa = resphres[i];
		}
		discrete[sphres+j].n = 0;
	...
	for (pose=1; pose<=poses; pose++)
	{
		...
		for (nodeno=0; nodeno<pathnodes; nodeno++)
		{
			...
			if (waters)
			{
				for (k=0; waters[k]; k++)
				{
					if (waters[k]->lastbind > 15)
					{
						n = waters[k]->get_atom_count();
						for (l=0; l<n; l++)
						{
							waters[k]->get_atom(l)->stream_pdb_line(
								pdbdat,
								8000+k*3+l
							);
						}
						cout << waters[k]->lastbind << endl;
					}
					else cout << "BAD " << waters[k]->lastbind << endl;
				}
			}
