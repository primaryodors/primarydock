...
APPS=$(BINDIR)/primarydock $(BINDIR)/interpreter
...
$(BINDIR)/interpreter: src/interpreter.cpp $(OBJS) $(OBJDIR)/aminoacid.o $(OBJDIR)/protein.o
...
molecule_report: test/molecule_test
	...
	./test/molecule_test 'NCCCC=O' 'NCCCC=O' 10 | sed '/^#/d' >test/molecule_test1.received.txt # ignore lines starting with #
