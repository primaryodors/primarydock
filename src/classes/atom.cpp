...
bool Atom::move(Point* pt)
{
    ...
    if (pdisnanf(pt->x) || pdisnanf(pt->y) || pdisnanf(pt->z))
    {
        return false;
    }
...
bool Atom::move_rel(SCoord* v)
{
    ...
    if (pdisnanf(v->phi) || pdisnanf(v->theta) || pdisnanf(v->r))
    {
        return false;
    }
...
void Atom::fetch_bonds(Bond** result)
{
    ...
    int i;
    if (!geometry || geometry<0 || pdisnanf(geometry)) geometry=4;
    try
    {
        for (i=0; i<geometry; i++)
        {
